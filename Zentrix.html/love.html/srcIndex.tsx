sssimport { useEffect, useRef, useState } from "react";
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import confetti from "canvas-confetti";
// Note: Assuming these components exist in the paths below
import { Button } from "@/components/ui/button"; 
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "sonner"; // Sonner library used for toasts
...
const Index = () => {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      // Animate sections on login (Home Page)
      ...
      // Animate cards
      gsap.utils.toArray([".feature-card", ".service-card"]).forEach((card: any, index: number) => {
        gsap.from(card, {
          y: 40,
          opacity: 0,
          duration: 0.8,
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Login animation and state change
  ...
  // --- LOGIN PAGE RENDER (Jab isLoggedIn false hai) ---
  if (!isLoggedIn) {
    return (
  ...
  // --- DOWNLOADS PAGE RENDER (Jab showDownloads true hai) ---
  if (showDownloads) {
    return (
  ...
  // --- HOME PAGE RENDER (Jab isLoggedIn true aur showDownloads false hai) ---
  const features = [
    {
